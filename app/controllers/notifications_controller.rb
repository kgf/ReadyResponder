class NotificationsController < ApplicationController
  ...
  def show
    @individuals = Person.where id: @notification.individuals
  ...
  # Use callbacks to share common setup or constraints between actions.
    ...
    def set_form_values
      ...
      @dept_choices = @event ? @event.departments.active.managing_people : Department.active.managing_people
      @purpose_choices = [["Request for Availability", "Availability"],
                          ["Info only", "FYI"],
                          ["Notify with Acknowledgment", "Acknowledgment"]]
      @individuals_choices = @event ? @event.eligible_people : Person.active
    end
  ...
  def notification_params
    params.require(:notification).permit(:subject, :body, :event_id, :status,
       ...
       :divisions, :department_ids => [], :individuals => [])
  end
end
